...
```
.
├── hierarchical_gnn.py          # HA-GNN implementation for relation extraction
├── z_nerd_tagger.py            # Z-NERD implementation for NER
...
This repository contains a comprehensive framework for scientific text analysis, featuring multiple approaches for both **Named Entity Recognition (NER)** and **Relation Extraction (RE)**. The repository is organized into modular components with baseline implementations and novel proposed solutions for processing scientific literature.

The main contributions include:

1. **Hierarchical Graph Neural Network (HA-GNN)** - A sophisticated approach for scientific relation extraction that incorporates hierarchical constraints and continuum abstraction fields
2. **Z-NERD Tagger** - A novel NER model using orthogonal semantic decomposition and multi-scale temporal convolutional attention  
3. **DP-ERE** - An additional relation extraction approach with positional learning markers

All models are designed specifically for processing scientific literature, with particular focus on the SciERC dataset for entity recognition and relation classification tasks.
...
```bash
python hierarchical_gnn.py --data_dir /path/to/scierc/ \
                          --train_file train.json \
                          --dev_file dev.json \
                          --test_file test.json \
                          --epochs 10 \
...
For questions about the implementation or research, please open an issue in this repository.